...
def collect_neighbor_node_representations_2D(representation, positions):
    ...
    neigh_num = input_shape[1]
    idxs = tf.range(0, limit=batch_size) # [batch,]
    idxs = tf.reshape(tf.tile(idxs, [neigh_num]), [batch_size,neigh_num,]) # [batch_size, neigh_num]
    positions = tf.stack((idxs,positions,), axis=2) # [batch_size, neigh_num, 2]
    return tf.gather_nd(representation, positions)
...
class GraphEncoder(object):
    def __init__(self, word_vocab=None, edge_label_vocab=None, char_vocab=None, is_training=True, options=None):
        ...
        with tf.variable_scope('graph_encoder'):
            ...
            passage_node_hidden = tf.zeros([batch_size, 1, options.neighbor_vector_dim])
            passage_node_cell = tf.zeros([batch_size, 1, options.neighbor_vector_dim])
            ...
            def _recurrence(passage_node_hidden, passage_node_cell, idx_var):
                ...
                # [batch_size, neighbor_size, neighbor_vector_dim]
                ...
                # [batch_size, 1, neighbor_vector_dim]
            ...
            passage_node_hidden, passage_node_cell, idx_var = tf.while_loop(loop_condition,
                    _recurrence, loop_vars, parallel_iterations=10,
                    shape_invariants=[
                        tf.TensorShape([None, None, options.neighbor_vector_dim]),
                        tf.TensorShape([None, None, options.neighbor_vector_dim]),
                        idx_var.get_shape(),])
